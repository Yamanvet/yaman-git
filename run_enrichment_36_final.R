#!/usr/bin/env Rscript
# CTVT: Enrichment — 36-sample protein-coding DEGs using EnrichR
# Proper MSTRG → gene_name mapping from merged GTF
...
# Load proper MSTRG → gene_name mapping
...
for (cname in contrasts) {
  ...
  if (!file.exists(f)) { cat("  Skipping\n"); next }
  
  degs <- read.csv(f, stringsAsFactors = FALSE, row.names = 1)
  degs$gene_id <- rownames(degs)
  degs$symbol <- symbol_map[degs$gene_id]
  
  # Remove NA/duplicate symbols, keep lowest padj
  ...
  for (direction in c("up", "down", "all")) {
    ...
    if (length(genes) < 5) {
      cat(sprintf("  %s: too few (%d), skip\n", direction, length(genes)))
      next
    }
    
    cat(sprintf("  EnrichR %s (%d genes)...\n", direction, length(genes)))
