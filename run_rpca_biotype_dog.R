#!/usr/bin/env Rscript
# CTVT: rPCA — Salmon tximport + biotype-filtered (protein-coding + known lncRNA) + ~ dog_id + timepoint
# Lightweight: only ~17k genes matching our actual DESeq2 analysis
...
# Load biotype tx2gene (protein-coding + known lncRNA only)
...
cat(sprintf("Biotype tx2gene: %d transcripts -> %d genes\n", 
            nrow(tx2gene_biotype), length(unique(tx2gene_biotype$gene_id))))
...
# tximport with biotype-filtered tx2gene
...
# VST (blind=FALSE: uses design to remove dog effect)
...
cat("\n═══════════════════════════════════════════════════════════════\n")
cat("  rPCA — Salmon + biotype-filtered + ~ dog_id + timepoint\n")
cat("═══════════════════════════════════════════════════════════════\n\n")
