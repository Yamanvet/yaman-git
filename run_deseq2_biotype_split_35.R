#!/usr/bin/env Rscript
# ====================================================================
# CTVT RNA-seq: DESeq2 Split by Biotype — 35 samples (T9N removed)
# Outlier: T9N_S26 — 3/5 methods flagged, clusters with Day0 not Recovered
# Design: ~ dog_id + timepoint (paired longitudinal)
# Includes: lfcShrink for publication-quality fold changes
# ====================================================================
...
# T9N_S26 removed: 5-method consensus outlier (clusters with Day0, r=0.898 with Recovered)
OUTLIERS_35 <- c("T9N_S26")
OUTDIR <- file.path(BASE, "deseq2_biotype_split_35")
dir.create(OUTDIR, showWarnings = FALSE, recursive = TRUE)

# ── 1. Sample metadata ─────────────────────────────────────────────
cat("═══════════════════════════════════════════════════════════════\n")
cat("  CTVT DESeq2 — Biotype-Split Analysis (35 samples)\n")
cat("  Removed: T9N_S26 (Dog9 Recovered — clusters with Day0)\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

all_samples <- basename(list.dirs(QUANT_DIR, recursive = FALSE, full.names = FALSE))
keep_samples <- setdiff(all_samples, OUTLIERS_35)
...
for (bt_name in names(biotype_runs)) {
  ...
  outdir <- file.path(OUTDIR, cfg$outfile)
  dir.create(outdir, showWarnings = FALSE, recursive = TRUE)
  ...
  for (cname in names(contrasts)) {
    ...
    # Standard results (unshrunken LFC)
    res <- results(dds, contrast = c("timepoint", c1, c2), alpha = 0.05)
    
    # LFC shrinkage (normal prior — works with contrast)
    ...
    # Merge shrunken LFC into results
    ...
    # Save significant DEGs with both LFCs
...
cat("\n\n═══════════════════════════════════════════════════════════════\n")
cat("  35-sample biotype-split DESeq2 complete!\n")
cat("═══════════════════════════════════════════════════════════════\n")